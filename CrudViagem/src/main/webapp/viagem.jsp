...
<body>
	...
	<div align="center" class="container">
		<form action="viagem" method="post">
			...
			<table>
				<tr>
					<td colspan="3"><label for="data">Codigo Viagem:</label> <input
						class="input_data" type="number" min="0" step="1" id="codigo"
					...
					<td colspan="4"><label for="data">Motorista:</label> <select
						class="input_data" id="motorista" name="motorista">
							...
							<c:forEach var="m" items="${motoristas }">
								<c:if
									test="${(empty viagem) || (m.codigom ne viagem.motorista.codigom) }">
									<option value="${m.codigom }"><c:out value="${m }" /></option>
								</c:if>
								<c:if test="${m.codigom eq viagem.motorista.codigom }">
									<option value="${m.codigom }" selected="selected"><c:out
											value="${m }" /></option>
								</c:if>
							</c:forEach>
					</select></td>
				...
				<tr>
					<td colspan="4"><label for="data">Hora Saida:</label> <input
						class="input_data" type="number" min="0" max="23" step="1"
			...
			<c:when
				...
				<c:if test="${not empty viagens}">
					<table class="table_round">
						<thead>
							<tr>
								<th style="text-align: center;">Codigo Viagem </th>
					...
					<table class="table_round">
						<thead>
							<tr>
								...
								<th style="text-align: center;">Placa</th>
								<th style="text-align: center;">Hora Saida</th>
